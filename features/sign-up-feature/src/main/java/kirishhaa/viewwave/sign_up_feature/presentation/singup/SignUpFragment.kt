        ...
        observeScreenState(binding.root, viewModel.state) { errorState ->
            when (errorState.error) {
                ...
                is PasswordsMatchException -> {
                    binding.etRepeatPassword.error =
                        resources.getString(incorrect_repeated_password)
                }
                else -> {
                    Toast.makeText(context, resources.getString(incorrect_data), Toast.LENGTH_SHORT)
                        .show()
                }
            }
